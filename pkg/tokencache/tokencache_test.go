...
import (
	...
	"github.com/intility/minctl/mocks"
...
func TestTokenCache_Replace(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			prepareStore: func(store *mocks.CredentialStore) {
				store.On("Get", "test").Return([]byte{}, nil).Once() // Expecting the Get function to be called once
			},
		...
		},
		{
			...
			prepareStore: func(store *mocks.CredentialStore) {
				store.On("Get", "test").Return(nil, errors.New("unknown err")).Once() // Expecting the Get function to be called once
			},
