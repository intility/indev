...
// ResourceSpans.
func Spans(sdl []tracesdk.ReadOnlySpan) []*tracepb.ResourceSpans { //nolint:funlen
	if len(sdl) == 0 {
		return nil
	}
	...
	for _, sd := range sdl {
		...
		// been seen and the appending we did above will be included it in the
...
func span(sd tracesdk.ReadOnlySpan) *tracepb.Span {
	...
	s := &tracepb.Span{ //nolint:exhaustruct
		TraceId:                tid[:],
		SpanId:                 sid[:],
	...
	if parentSpanID := sd.Parent().SpanID(); parentSpanID.IsValid() {
...
func links(links []tracesdk.Link) []*tracepb.Span_Link {
	...
	for _, otLink := range links {
		...
		sl = append(sl, &tracepb.Span_Link{ //nolint:exhaustruct
			TraceId:                tid[:],
			SpanId:                 sid[:],
